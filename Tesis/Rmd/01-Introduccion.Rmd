---
# subtitle: "Carrera de Investigación y Doctorado"
title: "Modelos de Demanda Sísmica Probabilística"
runningheader: "Aplicaciones del aprendizaje estadístico a la generación automática de modelos de predicción" # only for pdf output
subtitle: "Aplicaciones del aprendizaje estadístico a la generación automática de modelos de predicción " # only for html output
author: "Postulante: Alejandro Verri Kozlowski"
# date: "`r Sys.Date()`"
# site: bookdown::bookdown_site
# github-repo: "averriK/PhD"
# documentclass: book
# output:
#   bookdown::html_book:
#     theme: paper
...
### Facultad de Ingeniería Civil de la Universidad de Buenos Aires.  Programa de Investigación y Doctorado

### Supervisores: Dr. Raúl D. Bertero (Director) y Dr. Emilio Picasso (Co-Director)

# Introducción {.unnumbered}

## Estado Actual de la Práctica {.unnumbered}
...
En general, las variables aleatorias $R$ y $M$ suelen considerarse *iid* y $P\left [M=m^*,R=r^* \right ] \approx P\left [M=m^*\right ] P\left [R=r^*\right ]$ . Sin embargo, existen algunos escenarios en donde puedan estar fuertemente correlacionadas. Por ejemplo, en sitios cercanos a fallas activas, la distancia al punto más cercano de la falla $R$ suele estar correlacionada con la ubicación del área de ruptura y la función de distribución de las distancias requiere del conocimiento de todas las posibles áreas de ruptura, por lo que $P\left [R=r^*,R=r^* \right ] \approx P\left [R=r^*|M=m^*\right ] P\left [M=m^*\right ]$
...
$$ \begin{equation} 
\epsilon_{D}(d^*,i) \approx  \frac{ln\ d^*-ln [\eta_{D|I}(i)]}{\sigma_{ln \ D} }  \\ \epsilon_{I}(i^*,m,r) \approx  \frac{ln \ i^*-ln[\eta_{I|M,R}(m,r)]}{\sigma_{ln \ I}}
\end{equation}$$
...
$$ \begin{equation}  
\int^{i_{max}^*}_{i_{min}^*} \int^{r_{max}^*}_{r_{min}^*}  \int^{m_{max}^*}_{m_{min}^*}  \ \frac{\tilde\Phi \left[ \epsilon_{D}(d^*,i) \right] \phi \left[\epsilon_{I}(i,m,r) \right]  \ f_{R|M}(r,m) \ f_M(m)}{ i \ \sigma_{ln I}} \  dm \ dr \ di \leq \frac {-ln \left(1-p^* \right)}{\nu_o \ T_E}  
\end{equation}$$
...
## Limitaciones del estado actual de la práctica {.unnumbered}

Las principales limitaciones en la definición de modelos de estimación de la demanda sísmica probabilística, se originan principalmente en cuatro puntos

### **Limitaciones en la caracterización del movimiento sísmico** {.unnumbered}
...
### **Limitaciones en la consideración de la influencia del Sitio** {.unnumbered}
...
### **Limitaciones en la hipótesis de ergodicidad** {.unnumbered}
...
### *Limitaciones en la selección de sismos de diseño* {.unnumbered}
...
La motivación del trabajo de investigación parte de las limitaciones identificadas en el capitulo anterior, a saber:
...
La primera parte (este capítulo) presenta una introducción del problema fundamental del diseño sísmico basado en el desempeño en sistemas dinámicos complejos. La sección 1 revisa el estado actual del arte a ravés de las características de un modelo conceptual de predicción de la demanda sísmica. La sección 2, identifica las principales lilmitaciones del estado actual de la práctica y la sección 3, finalmente (esta sección) se plantea el enfoque para abordar esta necesidad y propone la organización y los objetivos del trabajo de investigación

### **Capítulo II: Caracterización del Movimiento Sísmico** {.unnumbered}

La segunda parte estará enfocada básicamente en la caracterización del movimientp sísmico y se desarrolla en cinco secciones, que consistirán en compilar, procesar, regularizar, normalizar y finalmente parametrizar las series temporales
...
### **Capítulo III: Predicción de la Demanda Sísmica** {.unnumbered}

La tecera parte del trabajo (Capítulo III) estará enfocada en la implementación de un modelo local de predicción del movimiento sísmico y un modelo de demanda sísmica. En la última sección, se presenta un problema de aplicación de la práctica y se comparan los resultados con el problema conceptual planteado en el Cap. I
...
### **Capítulo IV: Cierre** {.unnumbered}

En la última parte, se resumen los principales hallazgos y aportes de la metodología (Sección 1), se discuten los resultados obtenidos (Sección 2), se identifican limitaciones de las metodologías propuestas (sección 3) y finalmente se proponen algunas ideas de investigación para futuras aplicaciones de esta metodología.
...
**Alejandro Verri Kozlowski** (Ingeniero Civil)\
Ciudad de Mendoza, Agosto de de Abril de 2021. (Segundo año en cuarentena por COVID)
